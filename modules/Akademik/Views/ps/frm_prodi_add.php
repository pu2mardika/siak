...
echo '<input class="form-control" id="name" type="text" required="required" name="nm_prodi" placeholder="'. 
	 $this->lang->line('prodi_name').'" title = "'.$this->lang->line('prodi_name').'"/>';
echo form_error('nm_prodi','<br><label></label><span class="alert-warning">','</span>'); 
echo '</div>';
//desc 
echo '<div class="form-group input">';
echo "<label for='desc'>". $this->lang->line('prodi_desc')."</label>";
echo '<textarea id="desc" name="desc" class="form-control editor" rows="4" required="required"></textarea>';
echo form_error('desc','<span class="alert-warning">','</span>');
echo '</div>';
//jurusan
echo '<div class="form-group input">';
echo "<label for='id_jur'>". $this->lang->line('jur_name')."</label>";
echo form_dropdown('id_jur',$jurdd,'','class="form-control input-sm" required="required" id="id_jur" title="'.$this->lang->line('jur_name').'"'); 
echo form_error('id_jur','<span class="alert-warning">','</span>');
echo '</div>';
//grade
echo '<div class="form-group input">';
echo "<label for='jenjang'>". $this->lang->line('prodi_grade')."</label>";
echo form_dropdown('jenjang',$egrid,'','class="form-control input-sm" required="required" id="jenjang" title="'.$this->lang->line('prodi_grade').'"'); 
