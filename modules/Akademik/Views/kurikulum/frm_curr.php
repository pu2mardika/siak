...
$row=$curriculum;
$issued=nice_date(unix_to_human((is_int($row['issued'])||$row['issued']==0)?now():$row['issued']),"d-m-Y");
//$issued=tgl_fr_unix(($this->uri->segment(2,0)=="add_curr")?$row['issued']:now(),"d-m-Y");
...
echo '<input id="issued" type="text" class="form-control" required="required" name="issued" value = "'.$issued.'"/>';
echo form_error('issued','<span class="error">','</span>');
echo '</div>';
//curr_desc
echo '<div class="form-group input">';
echo "<label for='desc'>". $this->lang->line('curr_desc')."</label>";
echo '<textarea id="desc" name="curr_desc" rows="5" class="form-control editor" required="required">'.
$row['curr_desc'].'</textarea>';
echo form_error('curr_desc','<span class="error">','</span>');
echo '</div>';
//curr_l_duration
echo '<div class="form-group input">';
echo '<label for= "l_duration">'. $this->lang->line('curr_l_duration').'</label>';
echo '<input id="l_duration" type="text" class="form-control" required="required" name="l_duration" placeholder="1" title = "'.$this->lang->line('curr_l_duration').'" value = "'.$row['l_duration'].'"/>';
...
echo form_dropdown('curr_system',$ls_options,$row['curr_system'],'class="form-control input-sm" required="required" id="curr_system" title="'.$this->lang->line('curr_system').'"'); 
