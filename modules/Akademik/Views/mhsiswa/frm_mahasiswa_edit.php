...
<?php 
	...
	$ids=$this->kriptograf->paramEncrypt($row->nim,$keys);
?>
<div class="title"><?php echo $this->lang->line('mhs_edit');?></div>
<div><?php echo $row->nama_mhs." [".$row->nim."]";?></div>
...
<?=$this->pquery->form_remote_tag(array(
	'url'=>site_url('academic/mahasiswa/edit_profil/'.$ids), 
	'id'=>'mhs_edit',
	'autocomp'=>'on',							
...
echo '<input id="pname" type="text" required="required" name="nama_mhs" placeholder="'. 
	 $this->lang->line('mhs_name').'" title = "'.$this->lang->line('mhs_name').'" value = "'.$row->nama_mhs.'"/>';
echo form_error('nama_mhs','<br><label></label><span class="error">','</span>')."<br/>"; 

echo "<label for='nim_id'>". $this->lang->line('mhs_id')."</label>";
echo '<span>'.$row->nim.'</span>'.form_hidden('nim',$row->nim).'<br>'; 
...
echo '<input id="tmp_lahir" type="text" required="required" name="temp_lahir" placeholder="'. 
	 $this->lang->line('mhs_pob').'" title = "'.$this->lang->line('mhs_pob').'" value = "'.$row->temp_lahir.'"/>';
...
$th=date('Y')-17;
$thn=$th.'-'.date('m-d');
echo '<input id="tgl_lahir" type="date" required="required" name="tgl_lahir" max="'.$thn.'" value = "'.$row->tgl_lahir.'"/>';
...
echo form_dropdown('jk',$options, $row->jk);
...
echo '<input id="agama" type="text" required="required" name="agama" placeholder="'. 
	 $this->lang->line('mhs_religi').'" title = "'.$this->lang->line('mhs_religi').'" value = "'.$row->agama.'"/>';
...
echo "<label for='alam'>". $this->lang->line('mhs_addr')."</label>";
...
echo '<input id="ayah" type="text" required="required" name="alamat" placeholder="'. 
	 $this->lang->line('mhs_addr').'" title = "'.$this->lang->line('mhs_addr').'" value = "'.$row->alamat.'"/>'; 
...
echo '<input id="phone" type="text" required="required" name="phone" placeholder="'. 
	 $this->lang->line('mhs_phone').'" title = "'.$this->lang->line('mhs_phone').'" value = "'.$row->phone.'"/>'; 
echo form_error('phone','<span class="error">','</span>')."<br/>";

echo "<label for='email'>e-mail</label>";
echo '<input id="email" type="email" required="required" name="email" placeholder="email@example.com" title = "e-mail" value = "'.$row->email.'"/>';
...
echo form_dropdown('kode_prodi',$cmb_prodi,$row->kode_prodi);
echo form_error('kode_prodi','<span class="error">','</span>')."<br/>";

echo "<label for='asal_sek'>". $this->lang->line('mhs_school')."</label>";
echo '<input id="asal_sek" type="text" required="required" name="kode_asal_sek" placeholder="SMA N 1 Amlapura" title = "'.$this->lang->line('mhs_school').'" value = "'.$row->kode_asal_sek.'"/>';
echo form_error('kode_asal_sek','<span class="error">','</span>')."<br/>";

echo "<label for='ijazah'>". $this->lang->line('mhs_ijazah_no')."</label>";
echo '<input id="ijazah" type="text" name="no_ijazah" placeholder="'.$this->lang->line('mhs_ijazah_no').'" title = "'.$this->lang->line('mhs_ijazah_no').'" value = "'.$row->no_ijazah.'"/>'; 
...
echo '<input id="tgl_ijz" type="date" name="tgl_ijazah" placeholder="'. 
	 $this->lang->line('mhs_ijazah_tgl').'" title = "'.$this->lang->line('mhs_ijazah_tgl').'" value = "'.$row->tgl_ijazah.'"/>';
...
echo form_dropdown('stsawal',$options,$row->sts_awl_mhs);
echo form_error('sts_awl_mhs','<span class="error">','</span>')."<br/>";

echo "<label for='tgl_reg'>".$this->lang->line('mhs_date_of_reg')."</label>";
$data = array('name'=>'tgl_lahir','id'=>'tgl_lahir','size'=>40, 'value'=>set_value('tgl_masuk'));
echo '<input id="tgl_reg" type="date" required="required" name="tgl_masuk" placeholder="'. 
	 $this->lang->line('mhs_date_of_reg').'" title = "'.$this->lang->line('mhs_date_of_reg').'" value = "'.$row->tgl_masuk.'"/>';
...
echo form_dropdown('cur_status',$options,$row->cur_status) ."<br/>";
...
echo '<input id="ayah" type="text" required="required" name="nm_ayah" placeholder="'. 
	 $this->lang->line('mhs_father_name').'" title = "'.$this->lang->line('mhs_father_name').'" value = "'.$row->nm_ayah.'"/>';
...
echo '<input id="payah" type="text" required="required" name="pek_ayah" placeholder="'. 
	 $this->lang->line('mhs_father_job').'" title = "'.$this->lang->line('mhs_father_job').'" value = "'.$row->pek_ayah.'"/>';
...
echo '<input id="ibu" type="text" required="required" name="nm_ibu" placeholder="'. 
	 $this->lang->line('mhs_mother_name').'" title = "'.$this->lang->line('mhs_mother_name').'" value = "'.$row->nm_ibu.'"/>';
...
echo '<input id="pibu" type="text" required="required" name="pek_ibu" placeholder="'. 
	 $this->lang->line('mhs_mother_job').'" title = "'.$this->lang->line('mhs_mother_job').'" value = "'.$row->pek_ibu.'"/>';
...
echo '<input id="almt_ortu" type="text" required="required" name="alamat_ortu" placeholder="'. 
	 $this->lang->line('mhs_parents_addr').'" title = "'.$this->lang->line('mhs_parents_addr').'" value = "'.$row->alamat_ortu.'"/>';
