$(document).ready(function() {
				...
				if (confirmed) {
					...
					$.ajax({
						...
						success : function(data) {
							...
							$button.removeClass('disabled').removeAttr('disabled');
							show_alert('error', 'Ajax Error !!!', xhr.responseJSON.message + '<br/><strong>Note</strong>: Silakan cek detail error di console browser');
							console.log(xhr );
						}
					})
				}
	...
	$gallery_container.delegate('.btn-delete-all', 'click', function(e) 
	{
		...
		$loader = $('<span class="spinner-border spinner-border-sm ml-2 spinner" role="status" aria-hidden="true"></span>').appendTo($this);
		$(window).off('scroll');
